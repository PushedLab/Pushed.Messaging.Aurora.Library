...
class OpenGLTexture {
 public:
  // A callback used for retrieving surface descriptors.
  typedef std::function<bool(FlutterDesktopOpenGLTextureDescriptor*)>
      ObtainDescriptorCallback;

  OpenGLTexture(ObtainDescriptorCallback obtain_descriptor_callback)
      : obtain_descriptor_callback_(obtain_descriptor_callback) {}

  // Returns the callback-provided FlutterDesktopOpenGLTextureDescriptor that
  // contains the surface handle.
  bool ObtainDescriptor(FlutterDesktopOpenGLTextureDescriptor* descriptor) const {
    return obtain_descriptor_callback_(descriptor);
  }
...
// GpuSurfaceTexture is not currently implemented.
typedef std::variant<PixelBufferTexture, GpuSurfaceTexture, OpenGLTexture> TextureVariant;
