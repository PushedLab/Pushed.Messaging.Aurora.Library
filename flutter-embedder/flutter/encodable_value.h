...
class EncodableValue : public internal::EncodableValueVariant {
 public:
  ...
  // Fixed bug with nullptr on GCC-8
  EncodableValue(std::nullptr_t) : super() {}

  // GCC-8 does not generate a default copy constructor
  EncodableValue(const EncodableValue& v) : super(v) {}
  ...
  // Convenience method to simplify handling objects received from Flutter where
  // the values is EncodableMap. Useful for working with MethodChannel. This
  // function returns null if value is not EncodableMap.
  const EncodableMap* GetMapValue() const {
    if (std::holds_alternative<EncodableMap>(*this)) {
      return &std::get<EncodableMap>(*this);
    }
    return nullptr;
  }

  // Method to simplify handling values of EncodableMap. Useful for working with
  // MethodChannel. This function returns std::nullopt if value is not
  // EncodableMap or map is not contains value with name in argument.
  template <typename T>
  std::optional<T> GetValue(const std::string& name) const {
    ValidateArgumentType<T>();

    const auto* map = GetMapValue();
    if (!map) {
      return std::nullopt;
    }
 ...
 private:
  ...
  constexpr void ValidateArgumentType() const {
    ...
    static_assert(
        ...
        std::is_same_v<T, EncodableList> ||
        std::is_same_v<T, EncodableMap> ||
        std::is_same_v<T, CustomEncodableValue> ||
        std::is_same_v<T, std::vector<float>>,
        "attempts to get the argument with unsupported type from EncodableValue");
    /* clang-format on */
  }
};
