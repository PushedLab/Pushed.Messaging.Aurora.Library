...
typedef enum {
  ...
  // A platform-specific OpenGL texture.
...
// Supported OpenGL pixel formats.
typedef enum {
  // Represents a 32-bit RGBA color format with 8 bits each for red, green, blue
  // and alpha.
  kFlutterDesktopOpenGLPixelFormatRGBA8888,
} FlutterDesktopOpenGLPixelFormat;
...
typedef struct {
  ...
  size_t height;
  // The pixel format which might be optional depending on the surface type.
  FlutterDesktopOpenGLPixelFormat format;
  // Callback invoked (on an engine managed thread) that asks the embedder to
  // collect the texture.
  void (*release_callback)(void* release_context);
  // Opaque data passed to |release_callback|.
  void* release_context;
} FlutterDesktopOpenGLTextureDescriptor;
...
// The OpenGL texture callback definition provided to the Flutter engine
//  to obtain the surface. It is invoked with the |user_data| held by
// |FlutterDesktopGpuSurfaceTextureConfig|.
typedef bool (*FlutterDesktopOpenGLTextureCallback)(
    void* user_data,
    FlutterDesktopOpenGLTextureDescriptor* descriptor);
...
typedef struct {
  ...
  union {
    ...
    FlutterDesktopOpenGLTextureConfig opengl_texture_config;
  };
